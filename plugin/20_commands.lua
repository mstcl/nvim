...
---@type table
_G.toggle_registry = _G.toggle_registry or {} -- Unified Toggle command with extensible registry for plugin toggles

---@type table
_G.mode_registry = _G.mode_registry or {} -- Unified Mode command with extensible registry
...
_G.helpers.later(function()
	...
	create_user_command("YankCommitHash", function()
	...
	create_user_command("LspInfo", function() vim.cmd("checkhealth vim.lsp") end, {})

	-- Clear screen with extra stuff
	create_user_command("Clear", function()
	...
	create_user_command("Toggle", function(opts)
		...
		if arg == "relative_number" or arg == "relativenumber" or arg == "rnu" then
			...
			vim.notify(
				"Relative number "
					.. (vim.wo.relativenumber and "enabled" or "disabled"),
				vim.log.levels.INFO
			)
	...
	local pair_mode_enter = function()
	...
	local pair_mode_leave = function()
	...
	local pair_mode_toggle = function()
	...
	create_user_command("Mode", function(opts)
	...
	create_user_command("Syntax", function(opts)
		...
		local set_treesitter = function(enable)
			if enable then
		...
		local set_semantic = function(enable)
		...
		elseif target == "all" or target == "" then
			...
			elseif action == "disable" or action == "off" or action == "0" then
				...
				vim.notify(
					"Invalid action. Use: enable | disable",
					vim.log.levels.ERROR
				)
			end
		else
	...
	create_user_command("Pack", function(opts)
		...
		if cmd == "update" or cmd == "up" then
			if rest == "" then
				...
				if #plugins == 0 then
					vim.notify(
						"No valid plugin names provided",
						vim.log.levels.ERROR
					)
					return
				end
