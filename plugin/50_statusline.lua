...
_G.helpers.now(function()
			...
			vim.system(
				...
				function(result)
					...
					local function parse_git_status(output)
						...
						local ahead =
							tonumber(branch_info:match("ahead (%d+)") or "")
						local behind =
							tonumber(branch_info:match("behind (%d+)") or "")
	...
	local git_diffs = function()
		...
		if diffs ~= nil then
			...
			if diffs.changed ~= 0 then
				table.insert(
					items,
					set_hl("~" .. tostring(diffs.changed), "MoreMsg")
				)
			end
			...
			if diffs.changed ~= 0 then
				table.insert(
					items,
					set_hl("-" .. tostring(diffs.removed), "MoreMsg")
				)
			end
	...
	_G.statusline.components.filepath = function()
		...
		local root =
			set_hl(vim.fn.fnamemodify(vim.fn.getcwd(), ":t"), "SpecialComment")
		local secondary = ""
